...
```
```
In the graph above, you can notice that the avg homeruns per year of Hall of Famers is lower than the rest of the league.
This would indicate that Homeruns are not a crucial statistic in the game of baseball to get you into the Hall of Fame.
...
```{r}
AvgHS%>%
ggplot(aes(x = playerID, y = Salary))+
  geom_point()
```
```{r}
AvgS%>%
