...
HallOfFamerHomies <-
  BattingAndHallofFame %>%
    summarise(AvgHomeRunsPerPlayerHOF = mean(H))
...
LeagueFromAmerican <-
  BattingAndHallofFame %>%
    filter(lgID == "AA") %>%
    summarise(TotalFromAmerican = n())

LeagueFromNational <-
  BattingAndHallofFame %>%
    filter(lgID == "NL") %>%
    summarise(TotalFromNational = n())
...
The data for this graph counts the amount of times players who were inducted into the Hall Of Fame played in the All Star Game. With this, we made a bar chart of the amount of players who played total amounts. Example: only 1 player played 26 All Star Games and was inducted into the Hall Of Fame, so the bar graph indicates 1 at the x-pposition of 26. There is an included average bar to show the average amount of All Star apperances there have been by Hall Of Fame players.
```{r}
